...
def runMailbox(name="mailbox", base="", alias="mailbox", bran="", http=None, expire=0.0,
    ...
    cf = _makeConfiger(name=name, configDir=configDir, configFile=configFile)
    hby = _openHabery(name=name, base=base, bran=bran, cf=cf)
    ...
    try:
        http = int(http) if http is not None else urlparse(url).port
    except ValueError:
        http = urlparse(url).port
...
def _makeConfiger(name, configDir="", configFile=None):
...
def _openHabery(name, base="", bran="", cf=None, temp=False):
...
def _ensureHab(hby, alias, requireConfig=False):
    """Get or create non-transferable AID Hab for Mailbox and its HTTP URL"""
    hab = hby.habByName(alias)

    if hab is None:
        print(f"config path is {hby.cf}")
    ...
    if not _mailboxIdentityComplete(hby, hab, url):
        _reconcileMailboxIdentity(hby, hab, url)
